export default class Popup {
    ...
    openPopup() {  // метод отвечают за открытие попапа.
   ...
   _closePopupByPressingEsc (evt) {  // метод содержит логику закрытия попапа клавишей Esc.
        if (evt.key === 'Escape') {
          const openedPopup = document.querySelector('.popup_opened') // находит открытый попап
          this.closePopup(openedPopup) // закрывает попап с помощью функции closePopup
        }
